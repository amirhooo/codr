...
API_TOKEN = '206005521:AAHlcmJh5rXU3S5MEHTMkRX39PLL93_VYJ0'
...
def on_user_joins(m):
	...
	if m.content_type == 'new_chat_participant':
		if m.new_chat_participant.id == bot.get_me().id:
			...
			groupid = m.chat.id
			bot.send_message('-110293279', "# DEBUG # " + "Bot got invited to the group " + str(groupname) + "(" + str(groupid) + ")", parse_mode="HTML")
...
def credits(m):
	cid = m.chat.id
	bot.send_message(cid, "@Code_X", parse_mode="Markdown")
