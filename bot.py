...
API_TOKEN = '200262383:AAEC342tXsuuXroWPXCD3kveyD8I6R_fobQ'
...
def on_user_joins(m):
	...
	if m.content_type == 'new_chat_participant':
		if m.new_chat_participant.id == bot.get_me().id:
			...
			groupid = m.chat.id
			bot.send_message('-135445491', "# DEBUG # " + "Bot got invited to the group " + str(groupname) + "(" + str(groupid) + ")", parse_mode="HTML")
...
def credits(m):
	cid = m.chat.id
	bot.send_message(cid, "amir w.h", parse_mode="Markdown")
